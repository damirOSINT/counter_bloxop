...
local HitParts = {"Head", "UpperTorso", "LowerTorso", "RightLeg", "LeftLeg"} -- Возможные части тела
...
RunService.RenderStepped:Connect(function()
    ...
    if Holding == true and AimOn == true then
        ...
        if target and target.Character then
            local hitPart = HitParts[math.random(#HitParts)] -- Выбираем случайную часть тела
            if target.Character:FindFirstChild(hitPart) then
                ts:Create(Camera, TweenInfo.new(_G.Sensitivity, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {CFrame = CFrame.new(Camera.CFrame.Position, target.Character[hitPart].Position)}):Play()
            end
        end
    end
end)